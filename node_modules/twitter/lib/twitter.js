...
Twitter.prototype.__request = function(method, path, params, callback) {
  ...
  if (method === 'get') {
    ...
    // Gérer différemment si l'envoi concerne des médias
    if (typeof params.media !== 'undefined') {
      options.formData = params;
      delete options.headers['Content-Type']; // Supprimer le Content-Type pour multipart/form-data
    } else {
  ...
  // Pass form data if post
if (method === 'post') {
  // Définir l'en-tête Content-Type pour les requêtes POST avec un corps
  options.headers = {
    'Content-Type': 'application/json'
  };

  // Convertir le corps en JSON seulement si ce n'est pas déjà un FormData
  if (typeof params !== 'undefined' && typeof params.media === 'undefined') {
    options.body = JSON.stringify(params);
  } else {
    // Pour les médias (comme les images ou vidéos), utiliser formData
    options.formData = params;
    delete options.headers['Content-Type']; // Laisser la bibliothèque définir le Content-Type pour FormData
  }
}
